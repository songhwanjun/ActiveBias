...
def active_bias(gpu_id, input_reader, total_epochs, batch_size, lr_boundaries, lr_values, optimizer_type, noise_rate, warm_up=15, smoothness=0.2, log_dir="log"):
    ...
    with graph.as_default():
        with tf.device('/gpu:' + str(gpu_id)):
            with tf.Session(config = config) as sess:
                train_images, train_labels = input_reader.data_read(batch_size, train = True)
                test_images, test_labels = input_reader.data_read(batch_size, train = False)
                ...
                train_batch_patcher.bulk_load_in_memory(sess, train_images, train_labels)
                test_batch_patcher.bulk_load_in_memory(sess, test_images, test_labels)

                # give noise on data set
                train_batch_patcher.set_noise(noise_rate)
